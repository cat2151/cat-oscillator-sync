...
naudiodon でも内部バッファは約170msから減らすことができません。これは以下の複数の要因が組み合わさった結果です：

- PortAudio 自体の内部バッファリング
- Node.js のネイティブバインディングのオーバーヘッド
- OS のオーディオスタックの遅延

これら全ての要因が合わさって、**約170msの総合的なレイテンシー**となります。
...
- naudiodon（PortAudio）自体はクロスプラットフォームですが、このプロジェクトではWindows専用として実装しています
